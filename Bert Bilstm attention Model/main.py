...
if __name__ == '__main__':
    ...
    X_ids_train, X_mask_train, Y_train, X_ids_test, X_mask_test, Y_test = pretreat.bert_train_test_split(X_ids, X_mask, mutil_labels, test_size=0.3, shuffle=True, random_state=1)  # train_test_split仅应该关于离散或分类变量进行分层
    X_ids_valid, X_mask_valid, Y_valid, X_ids_test, X_mask_test, Y_test = pretreat.bert_train_test_split(X_ids, X_mask, mutil_labels, test_size=0.1, shuffle=True, random_state=1)  # train_test_split仅应该关于离散或分类变量进行分层

    # add 测试，找到预测不对的文本是为什么
    # X_train, y_train = X, mutil_labels
    # train_data = torch.utils.data.TensorDataset(X_train, y_train)  # (([[],])，([[],]))
    # test_iter = torch.utils.data.DataLoader(train_data, batch_size=config.batch_size, shuffle=False, drop_last=True)
